...
# ዳታቤዙን ለማጽዳትና በአዲስ ለመፍጠር
@app.route('/init_db')
def init_db():
    db.drop_all() # የድሮውን ስህተት ያለበትን ዳታቤዝ ያጠፋዋል
    db.create_all() # አዲሱን score ያለበትን ይፈጥራል
...
def submit_exam():
    ...
    if user_rec:
        user_rec.score = 4 # ውጤቱን 4 አደረግነው
        db.session.commit()
